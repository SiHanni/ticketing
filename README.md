...
> **순간 폭증 트래픽** 상황에서도 안정적으로 좌석을 관리·처리하는 **티켓 예매 시스템**을 직접 설계·구현한 프로젝트입니다.  
> Kafka·Redis·NestJS 기반의 **대기열·분산락·재시도 처리** 등 실전형 패턴을 구현하고, Artillery/k6로 **순간 폭증 부하를 재현**하여 성능까지 검증했습니다.

## 개요

> **목표**: 순간적으로 몰리는 고트래픽 처리가 가능한 환경과 안정적인 티켓 예매 시스템을 구현하고, 성능 병목 요소를 직접 설계하고 테스트 한다.
...
- ✅ **순간 폭증 트래픽(초당 2,000명 유입)** 부하 테스트에서 **요청 실패율 0%, 1,000석 정확히 확정 처리**
...
1. kafka, redis compose up
2. kafka 토픽 생성

- reservation.requested, reservation.paid
- retry.reservation.paid(1~3)
- retry.reservation.requested(1~3)
- reservation.dlq

3. pnpm install, build, 서비스별 pnpm install, build
4. 현재 시점 기준 활성화된 이벤트 데이터 추가
5. 공연장 및 공연작 별 좌석 데이터 추가
